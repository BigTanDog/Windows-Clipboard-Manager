...
| 上限 | 条数上限（默认 100 条）+ 磁盘占用上限（默认 500MB，可选 200MB/500MB/1GB/2GB/不限）；超限按「非收藏优先 → volume 大者优先 → 同级最旧优先」淘汰；只剩收藏仍超限时提示用户，不自动删除 |
...
| 设置 | 条数/磁盘上限、热键、开机自启（含延迟启动）、按进程名排除、是否记录图片、脱敏开关、主题 |
| 托盘 | 显示面板 / 设置 / 清空历史（二次确认）/ 开机自启 / 退出；图标由代码运行时生成（40 行 ICO，不依赖外部资源）；资源管理器重启后自动重挂 |
| 主题 | 跟随系统（`AppsUseLightTheme`）/ 强制浅色 / 强制深色；收到 `WM_SETTINGCHANGE` 即时切换 |
