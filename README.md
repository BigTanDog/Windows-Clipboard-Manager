...
| 交互 | 点击面板外部自动隐藏（附加项 B-01，默认开，可在设置里关掉）；单击条目默认只选中，双击 / `Enter` 粘贴（可在设置里改回"单击即粘贴并收起"）；Esc / 热键 / 关闭按钮同样可收起 |
| 亚克力 | 面板支持系统模糊背景，强度 0–100 可调（附加项 B-04）：0 = 不透明，越大越透、模糊越明显；设置页拖动即时预览，取消自动回退。实现走 `ACCENT_ENABLE_BLURBEHIND`（Windows 11 上"亚克力(4)"实测不产生模糊，见 `docs/产品计划文档.md` §9.2） |
...
| A | `artifacts/大体积-免运行时/ClipboardManager.exe` | **64.39 MB** | 免运行时、任意 Windows x64（U 盘/新机器直接双击） |
